...
var mysensors = require('./mysensors.js');
...
const Datastore = require('nedb-promises');                      //https://github.com/bajankristof/nedb-promises
var nodemailer = require('nodemailer');                         //https://github.com/andris9/Nodemailer
var http = require('http');
var url = require('url');
let db = Datastore.create({ filename: path.join(__dirname, dbDir, settings.database.name.value), autoload: true });       //used to keep all node/metric data
...
if (settings.mqtt.enabled.value === `true`) {
  ...
  var mqtt_Options = {
    ...
    username: settings.mqtt.username.value, //the username required by your broker, if any
    password: settings.mqtt.password.value, //the password required by your broker, if any
    will: { topic: settings.mqtt.state_topic.value, payload: mqtt_client_id + ' disconnected', qos: 1, retain: false },
    //qos : settings.mqtt.qos.value
  }

  if (DEBUG_MQTT) console.info('Starting MQTT...');
  var mqtt_client = mqtt.connect('mqtt://' + settings.mqtt.host.value, mqtt_Options);

  function mqttSubscribe() {
    if (settings.mqtt.subscribe_topic.value != '') {
      if (DEBUG_MQTT) console.info(`Subscribing MQTT topic${settings.mqtt.subscribe_topic.value}`);
      mqtt_client.subscribe(settings.mqtt.subscribe_topic.value + '/#');
    }
    if (settings.mqtt.mysensors_subscribe_topic.value != '') {
      if (DEBUG_MQTT) console.info(`Subscribing MQTT topic${settings.mqtt.mysensors_subscribe_topic.value}`);
      mqtt_client.subscribe(settings.mqtt.mysensors_subscribe_topic.value + '/#');
    }
  }
  ...
  mqtt_client.handleMessage = function (packet, done) {
    ...
    var message = '';
    //mysensors parsing
    if (packet.topic.includes(settings.mqtt.mysensors_subscribe_topic.value)) {
      if (DEBUG_MQTT) console.log('INCOMING MYSENSORS MQTT: ' + packet.topic + ' ' + packet.payload.toString());
      packet.topic = packet.topic.split(settings.mqtt.mysensors_subscribe_topic.value + "/")
      packet.topic = packet.topic[1];
      source = "MYSENSORS_MQTT";
      message = mysensors.processMySensors(packet.payload, packet.topic);
    } else { //moteino message parsing
      if (DEBUG_MQTT) console.log('INCOMING MQTT: ' + packet.topic + ' ' + packet.payload.toString());

      source = "MQTT";
      message = packet.payload.toString().replace(/\0/g);
    }
    processData(message, source, done);
  }

  // MQTT INCOMING
  // no backpressure support
  // mqtt_client.on('message', function (topic, message) {
  // 	console.info(`INCOMING MQTT:`); 
  // 	processData(message.toString().replace(/\0/g), false);
  // });
...
function mqttSend(msg) {
  if (DEBUG_MQTT) console.info('mqttSend(' + msg + `)`);
  mqtt_client.publish(settings.mqtt.publish_topic.value, msg);
}
...
if (settings.serial.enabled.value === `true`) {
  var openPort = (function f(reopen) {
    ...
    parser.on('data', function (data) { processData(data.replace(/\0/g, ''), settings.serial.port.value, () => { }); }); //replace nulls in received string
...
global.getGraphData = function (nodeId, sensorId, metricKey, start, end, exportMode) {
...
global.getStatisticData = function (nodeId, sensorId, metricKey, start, end, exportMode) {
  var sts = Math.floor(start / 1000); //get timestamp in whole seconds
  var ets = Math.floor(end / 1000); //get timestamp in whole seconds
  var logfile = path.join(__dirname, dbDir, dbLog.getLogName(nodeId, sensorId, metricKey));
  var graphData = dbLog.getStatistic(logfile, sts, ets, exportMode ? 100000 : settings.general.graphMaxPoints.value); //100k points when exporting, more points is really pointless
  var graphOptions = {};
  for (var k in metricsDef.metrics) {
    if (metricsDef.metrics[k].name == metricKey) {
      if (metricsDef.metrics[k].graphOptions !== undefined) {
  ...
  graphOptions.metricName = metricKey;
  return { graphData: graphData, options: graphOptions, graphOptions: graphData.graphOptions };
}
...
io.sockets.on('connection', function (socket) {
    ...
    db.find({ _id: node._id })
      .then((entries) => {
        if (entries.length == 1) {
          ...
          db.update({ _id: dbNode._id }, { $set: dbNode }).then(() => { if (DEBUG_DB) if (DEBUG_DB) console.log('UPDATENODESETTINGS records replaced:' /*+ numReplaced);*/); });
          socket.broadcast.emit('UPDATENODE', dbNode); //sent changes to all clients except sender socket
        }
      });
  });

  socket.on('UPDATEMETRICSETTINGS', function (nodeId, sensorId, metricKey, metric) {
    db.find({ _id: nodeId }).then((entries) => {
      if (entries.length == 1) {
        var dbNode = entries[0];
        dbNode.metrics[sensorId][metricKey].label = metric.label;
        dbNode.metrics[sensorId][metricKey].pin = (metric.pin == 1) ? Date.now() : metric.pin;
        dbNode.metrics[sensorId][metricKey].graph = metric.graph;
        db.update({ _id: dbNode._id }, { $set: dbNode })
          .then(() => { if (DEBUG_DB) console.log('UPDATEMETRICSETTINGS records replaced:'/* + numReplaced*/); });
  ...
  socket.on('ADDNODEGRAPH', function (nodeId, selectedMetrics) {
    var regexpSensorIdKey = /^(\d+)\:([a-z0-9!"#\$%&'()*+,.\/:;<=>?@\[\] ^_`{|}~-]+)/i; //sensorId(number):key
    db.find({ _id: nodeId }).then((entries) => {
      if (entries.length == 1) {
        var dbNode = entries[0];
        var newGraph = {};
        ...
        for (var arrayKey in selectedMetrics) {
          var value = selectedMetrics[arrayKey];
          if (regexpSensorIdKey.test(value)) {
            var metricTokens = regexpSensorIdKey.exec(value);
            var sensorId = metricTokens[1];
            var metricKey = metricTokens[2];
            if (Object.keys(dbNode.metrics[sensorId]).includes(metricKey)) //verify node metrics contains selected metric
            {
              if (newGraph[sensorId] === undefined) newGraph[sensorId] = [];
              newGraph[sensorId].push(metricKey);
  ...
  socket.on('DELETENODE', function (nodeId) {
    ...
    if (settings.general.keepMetricLogsOnDelete.value != 'true')
      db.find({ _id: nodeId }).then((entries) => {
        if (entries.length == 1) {
          var dbNode = entries[0];
          if (dbNode.metrics) {
            for (const sensorId in dbNode.metrics) {
              Object.keys(dbNode.metrics[sensorId]).forEach(function (mKey, index) { //syncronous/blocking call
                if (dbNode.metrics[sensorId][mKey].graph == 1)
                  dbLog.removeMetricLog(path.join(__dirname, dbDir, dbLog.getLogName(dbNode._id, sensorId, mKey)));
              });
            }
          }
        }
    ...
    db.remove({ _id: nodeId }).then(() => {
      console.log('DELETED entries: ' /*+ removedCount*/);

      //pull all nodes from the database and send them to client
      db.find({ _id: { $exists: true } }).then((entries) => {
  ...
  socket.on('DELETENODEMETRIC', function (nodeId, sensorId, metricKey) {
    db.find({ _id: nodeId }).then((entries) => {
      if (entries.length == 1) {
        var dbNode = entries[0];
        delete (dbNode.metrics[sensorId][metricKey]);
        db.update({ _id: dbNode._id }, { $set: dbNode }).then(() => { if (DEBUG_DB) console.info('DELETENODEMETRIC DB-Replaced:' /*+ numReplaced*/); });
        if (settings.general.keepMetricLogsOnDelete.value != 'true')
          dbLog.removeMetricLog(path.join(__dirname, dbDir, dbLog.getLogName(dbNode._id, sensorId, metricKey)));
        io.sockets.emit('UPDATENODE', dbNode); //post it back to all clients to confirm UI changes
      }
    });
  });
  ...
  socket.on('DELETENODEREQUEST', function (nodeId, requestKey) {
    db.find({ _id: nodeId }).then((entries) => {
      if (entries.length == 1) {
        ...
        db.update({ _id: dbNode._id }, { $set: dbNode }).then(() => { if (DEBUG_DB) console.info('DELETENODEREQUEST DB-Replaced:'/* + numReplaced*/); });
        io.sockets.emit('UPDATENODE', dbNode); //post it back to all clients to confirm UI changes
      }
    });
  });
  ...
  socket.on('CONTROLCLICK', function (control) {
    ...
    if (control.nodeType && control.controlKey && control.stateKey)
      if (metricsDef.motes[control.nodeType].controls[control.controlKey].states[control.stateKey].serverExecute != null) {
        db.findOne({ _id: control.nodeId }).then((node) => {
          if (node) {
  ...
  socket.on('SIMULATEDMESSAGE', function (str) {
    if (str) processData(str, "SIMULATED_MESSAGE", () => { });
  });
  ...
  socket.on('ADDNEWNODE', function (node) {
    if (isValidNodeId(node.nodeId)) {
      ...
      db.findOne({ _id: node.nodeId }).then((doc) => {
        if (doc == null) {
  ...
  socket.on('SUBMITNODEREQUEST', function (nodeId, reqName, reqValue, reqTimeout) {
    db.find({ _id: nodeId }).then((entries) => {
      if (entries.length == 1) {
        ...
        db.update({ _id: dbNode._id }, { $set: dbNode }).then(() => { if (DEBUG_DB) console.info('SUBMITNODEREQUEST DB-Replaced:' /*+ numReplaced*/); });
  ...
  socket.on('GETSTATISTICDATA', function (nodeId, sensorId, metricKey, start, end, exportMode) {
    var statisticalData = getStatisticData(nodeId, sensorId, metricKey, start, end, exportMode);
    socket.emit(exportMode ? 'EXPORTGRAPHDATAREADY' : 'STATISTICDATAREADY', statisticalData);
  });

  socket.on('GETGRAPHDATA', function (nodeId, sensorId, metricKey, start, end, exportMode) {
    var graphData = getGraphData(nodeId, sensorId, metricKey, start, end, exportMode);
  ...
  socket.on('GETMULTIGRAPHDATA', function (nodeId, multiGraphId, start, end, exportMode) {
    ...
    db.find({ _id: nodeId }).then((entries) => {
      if (entries.length == 1) {
        var dbNode = entries[0];
        if (dbNode.multiGraphs)//&& dbNode.multiGraphs[multiGraphId]) //TODO array changed to object , multiGraphId still useful?
        {
            ...
            if (multiGraphSensorsIdArray !== undefined)
              Object.keys(dbNode.metrics[sensorId]).forEach(function (mKey, index) {
                if (multiGraphSensorsIdArray[sensorId] && multiGraphSensorsIdArray[sensorId].includes(mKey) && dbNode.metrics[sensorId][mKey].graph == 1) {
                  ...
                  //var graphStats = getStatisticData(nodeId, sensorId, mKey, start, end, exportMode);
...
global.processData = function (data, source, done) {
  ...
  var regexpSensorId = /^(\d+)\:([a-z0-9!"#\$%&'()*+,.\/:;<=>?@\[\] ^_`{|}~-]+)/i; //sensorId(number):key:value
  ...
  if (match != null) {
    ...
    db.findOne({ _id: id }).then((dbEntry) => {
      var existingNode = {};
      ...
      if (existingNode.updated !== undefined && (Date.now() - existingNode.updated < 500) && msgHistory[id] == msgTokens) {
        ...
        if (DEBUG_MQTT) console.info(`handleMessage() - callback`);
        return;
      }
      ...
      {
        var matchingMetric;
        var sensorData = regexpSensorId.exec(match[0]);
        var sensorId = 0;
        if (sensorData) {
          sensorId = sensorData[1];
          match[0] = sensorData[2];
        }
        ...
        for (var metric in metricsDef.metrics) {
          if (metricsDef.metrics[metric].regexp.test(match[0])) {
            ...
            if (existingNode.metrics[sensorId] === undefined) existingNode.metrics[sensorId] = {};
            if (existingNode.metrics[sensorId][matchingMetric.name] == null) existingNode.metrics[sensorId][matchingMetric.name] = {};
            existingNode.metrics[sensorId][matchingMetric.name].label = existingNode.metrics[sensorId][matchingMetric.name].label || matchingMetric.name;
            existingNode.metrics[sensorId][matchingMetric.name].descr = existingNode.metrics[sensorId][matchingMetric.name].descr || matchingMetric.descr || undefined;
            existingNode.metrics[sensorId][matchingMetric.name].value = matchingMetric.value || determineValue(matchingMetric, tokenMatch);
            existingNode.metrics[sensorId][matchingMetric.name].unit = matchingMetric.unit || undefined;
            existingNode.metrics[sensorId][matchingMetric.name].updated = existingNode.updated;
            existingNode.metrics[sensorId][matchingMetric.name].pin = existingNode.metrics[sensorId][matchingMetric.name].pin !== undefined ? existingNode.metrics[sensorId][matchingMetric.name].pin : matchingMetric.pin;
            existingNode.metrics[sensorId][matchingMetric.name].graph = existingNode.metrics[sensorId][matchingMetric.name].graph !== undefined ? existingNode.metrics[sensorId][matchingMetric.name].graph : matchingMetric.graph;
            existingNode.metrics[sensorId][matchingMetric.name].statistic = matchingMetric.statistic;
            ...
            if (existingNode.metrics[sensorId][matchingMetric.name].graph == 1) {
              ...
              if (isNumeric(graphValue)) {
                ...
                var logfile = path.join(__dirname, dbDir, dbLog.getLogName(id, sensorId, matchingMetric.name));
                try {
      ...
      if (dbEntry == null) //it is an INSERT
      {
        if (settings.general.genNodeIfNoMatch.value == true || settings.general.genNodeIfNoMatch.value == 'true' || hasMatchedMetrics) {
          db.insert(entry)
            .then(() => {
              if (DEBUG_DB) console.info(`   [${id}] DB-Insert new _id:${id}`);
              done();
              if (DEBUG_MQTT) console.info(`handleMessage() - callback`);
            })
            .catch(() => {
              if (DEBUG_DB) console.error(`   [${id}] DB-Insert new _id:${id}`);
              done();
              if (DEBUG_MQTT) console.info(`handleMessage() - callback`);
            });
        ...
        db.update({ _id: id }, { $set: entry })
          .then(() => {
            if (DEBUG_DB) console.info(`[${id}] DB-Updated: processData('${data.replaceNewlines()}',${source}):entry=${JSON.stringify(entry)}`);
            done();
            if (DEBUG_MQTT) console.info(`handleMessage() - callback`);
          })
          .catch(() => {
            console.error(`[${id}] DB-Updated: processData('${data.replaceNewlines()}',${source}):entry=${JSON.stringify(entry)}`);
            done();
            if (DEBUG_MQTT) console.info(`handleMessage() - callback`);
          });
        io.sockets.emit('UPDATENODE', entry);
      }
      if (waitForDbUpdate === false) {
        done();
        if (DEBUG_MQTT) console.info(`handleMessage() - callback`);
      }
    ...
    while (match = regexTokenizedLine.exec(data)) //extract each whitespace separated token/value pair from serial data and process it
    {
      ...
      if (tokenMatch != null) {
        ...
        if (tokenMatch[1] == 'GATEWAYSTART') //indicates a GATEWAY (re)start
        {
          ...
          db.find({ _id: { $exists: true }, requests: { $exists: true } }).then((entries) => {
            var updates = [];
            var ts = Date.now();
...
global.schedule = function (node, eventKey) {
  ...
  //save to DB
  db.findOne({ _id: node._id }).then((dbNode) => {
    dbNode.events = node.events;
    db.update({ _id: dbNode._id }, { $set: dbNode }).then(() => { if (DEBUG_DB) console.info(`[${dbNode._id}] DB-Updated: schedule(${eventKey}):dbNode=${JSON.stringify(dbNode)}`); });
...
global.runAndReschedule = function (functionToExecute, nodeAtScheduleTime, eventKey) {
  ...
  db.findOne({ _id: nodeAtScheduleTime._id }).then((dbNode) => {
    try {
...
db.find({ events: { $exists: true } }).then((entries) => {
  var count = 0;
